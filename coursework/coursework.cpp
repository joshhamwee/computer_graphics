...
void filledRaytracedTriangles(){
  vec3 topLeftImagePlaneWorld(0,0,0);
  topLeftImagePlaneWorld.z = floor(camera.z - imagePlaneDistance);
  topLeftImagePlaneWorld.x = floor(camera.x + 0.5*WIDTH);
  topLeftImagePlaneWorld.y = floor(camera.y - 0.5*HEIGHT - 10);

  for (size_t j = 0; j < HEIGHT; j++) {
    for (size_t i = 0; i < WIDTH; i++) {
      vec3 rayDirection(topLeftImagePlaneWorld.x-i, topLeftImagePlaneWorld.y+j, imagePlaneDistance);
      rayDirection = rayDirection*glm::inverse(camera_orientation);
      rayDirection = glm::normalize(rayDirection);
      RayTriangleIntersection currentTriangle = getClosestIntersection(rayDirection);
      if (currentTriangle.distanceFromCamera != std::numeric_limits<float>::max()) {
        if (lightingType == 1) {
          float brightness = 2/(0.1f * PI * pow(currentTriangle.distanceFromLight,2));
          if (brightness > 1) {
            brightness = 1;
...
RayTriangleIntersection getClosestIntersection(vec3 rayDirection){
  ...
  for (size_t i = 0; i < triangles.size(); i++) {
    ...
    vec3 SPVector = camera - triangles[i].vertices[0];
    ...
    if (possibleSolution[1] > 0.0 && possibleSolution[2] > 0.0 && (possibleSolution[1] + possibleSolution[2] < 1.0) && possibleSolution[0] >= 0.0) {
      if(possibleSolution[0] < currentClosestIntersection.distanceFromCamera){
