  ...
  if (animationType == 0) {
    ...
    for (size_t triangle_count = 22; triangle_count < triangles.size(); triangle_count++) {
      for (size_t i = 0; i < 3; i++) {
        triangles[triangle_count].vertices[i].x = (triangles[triangle_count].vertices[i].x - (-1.12));
        triangles[triangle_count].vertices[i].y = (triangles[triangle_count].vertices[i].y - (3.14));
        triangles[triangle_count].vertices[i].z = (triangles[triangle_count].vertices[i].z - (-3.76));
...
void savePPM(){
  ...
  char filename[64];
  if (file_number < 10) {
    sprintf(filename, "out_files/0000%d.ppm", file_number);
  }
  else if (file_number > 99) {
    sprintf(filename, "out_files/00%d.ppm", file_number);
  }
  else{
    sprintf(filename, "out_files/000%d.ppm", file_number);
  }
  ...
  FILE * fp;
  const char *comment = "#binary ppm file";
